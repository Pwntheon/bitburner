import getAllServers from './lib/scan.js'

/** @param {NS} ns **/
export async function main(ns) {

	const servers = getAllServers(ns);
	const contracts = servers.map(hostname => ({ hostname, contracts: ns.ls(hostname, '.cct') }))
		.filter(s => s.contracts.length > 0)
		.map(s => s.contracts.map(c =>
		({
			contract: c,
			hostname: s.hostname,
			type: ns.codingcontract.getContractType(c, s.hostname),
			data: ns.codingcontract.getData(c, s.hostname)
		}))).flat();

	if (contracts.length === 0) return; // ns.tprint("No contracts found");
